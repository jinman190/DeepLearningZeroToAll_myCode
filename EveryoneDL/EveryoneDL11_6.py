...
#            L1 = tf.nn.relu(L1)
            L1 = tf.layers.conv2d(inputs=X_img, filters=32, kernel_size=[3,3], padding="SAME", activation=tf.nn.relu) #위 세줄이 이 한줄로

#            L1 = tf.nn.max_pool(L1, ksize=[1,2,2,1], strides=[1,2,2,1], padding='SAME') #풀링 subsampling
            L1 = tf.layers.max_pooling2d(inputs=L1, pool_size=[2,2], padding="SAME", strides=2) #윗 줄과 같은 뜻


#            W2 = tf.Variable(tf.random_normal([3, 3, 32, 64], stddev=0.01))
#            L2 = tf.nn.conv2d(L1, W2, strides=[1,1,1,1], padding='SAME') #output img shape (3,3) #합성곱 convolution
#            L2 = tf.nn.relu(L2)
            L2 = tf.layers.conv2d(inputs=L1, filters=64, kernel_size=[3,3], padding="SAME", activation=tf.nn.relu) #위 세줄이 이 한줄로

#            L2 = tf.nn.max_pool(L2, ksize=[1,2,2,1], strides=[1,2,2,1], padding='SAME') #풀링 subsampling
            L2 = tf.layers.max_pooling2d(inputs=L2, pool_size=[2,2], padding="SAME", strides=2) #윗 줄과 같은 뜻
...
#            L3 = tf.nn.relu(L3)
            L3 = tf.layers.conv2d(inputs=L2, filters=128, kernel_size=[3,3], padding="SAME", activation=tf.nn.relu) #위 세줄이 이 한줄로


#            L3 = tf.nn.max_pool(L3, ksize=[1,2,2,1], strides=[1,2,2,1], padding='SAME') #풀링 subsampling
            L3 = tf.layers.max_pooling2d(inputs=L3, pool_size=[2,2], padding="SAME", strides=2) #윗 줄과 같은 뜻
